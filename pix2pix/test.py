...
def test_model(args):
    ...
    # 加载训练好的模型
    G = init_AttentionUnet(args.G_ckpt).to(device)  # MRI → PET
    ...
    txt_path = os.path.join(synthesis_path, "pix2pix_results.txt")
    # 测试循环
    G.eval()
    with torch.no_grad():
        progress_bar = tqdm(
            ...
            with autocast(device_type="cuda", enabled=True):
                ...
                if args.synthesis:
                    ...
                    gen_path = os.path.join(
                        synthesis_path, f"{subject}_{pet_id}_{mri_id}_pix2pix.nii.gz"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_csv",
        default="/staff/limingchao/AD/csv/new/abeta-PET_dataset_3.csv",
        type=str,
    )
    parser.add_argument(
        "--cache_dir",
    ...
    parser.add_argument(
        "--output_dir",
        default="/staff/limingchao/AD/mycode/pix2pix/output/",
        type=str,
    )
    parser.add_argument(
        "--synthesis_dir",
        default="/staff/limingchao/AD/mycode/cycle_gan/synthesis",
        type=str,
    )
    ...
    parser.add_argument(
        "--G_ckpt",
        default="/staff/limingchao/AD/mycode/pix2pix/output/abeta_2025-04-21_09-59-49/best/G-ep-36.pth",
        type=str,
    )
    parser.add_argument(
        "--synthesis",
