...
def test_model(args):
    ...
    # 加载训练好的模型
    PET = init_AttentionUnet(args.disc_ckpt_m2p).to(device)  # MRI → PET
    ...
    txt_path = os.path.join(synthesis_path, "cycle_gan_results.txt")
    # 测试循环
    PET.eval()
    with torch.no_grad():
        progress_bar = tqdm(
        ...
        for step, batch in progress_bar:
            with autocast(device_type="cuda", enabled=True):
                ...
                sex = batch["Sex"][0]
                age = batch["Age"][0]
                pet_gen = PET(batch["mri"].to(device))
                ...
                if args.synthesis:
                    ...
                    gen_path = os.path.join(
                        synthesis_path, f"{subject}_{pet_id}_{mri_id}_cycle_gan.nii.gz"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        default="/staff/limingchao/AD/mycode/cycle_gan/output/",
        type=str,
    )
    parser.add_argument(
        "--synthesis_dir",
    ...
    parser.add_argument(
        "--disc_ckpt_m2p",
        default="/staff/limingchao/AD/mycode/cycle_gan/output/abeta_2025-04-19_19-37-28/best/PET-ep-72.pth",
        type=str,
    )
    parser.add_argument(
        "--disc_ckpt_p2m",
