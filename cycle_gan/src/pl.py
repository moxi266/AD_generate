...
class PatchAdversarialLoss(_Loss):
    ...
    def __init__(
        ...
        if criterion == AdversarialCriterions.BCE.value:
            ...
            if no_activation_leastsq:
                ...
                self.activation = get_act_layer(
                    name=("LEAKYRELU", {"negative_slope": 0.05})
                )
    ...
    def get_target_tensor(
        self, input: torch.FloatTensor, target_is_real: bool
    ) -> torch.Tensor:
        """
        ...
        label_tensor = torch.tensor(1, dtype=input.dtype, device=input.device).fill_(
            filling_label
        )
    ...
    def forward(
        ...
        for disc_ind, disc_out in enumerate(input):
            ...
            if (
                self.criterion == AdversarialCriterions.HINGE.value
                and not target_is_real
            ):
